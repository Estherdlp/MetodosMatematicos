...
function [c, iter] = secante(funcion,a,Tol)
  ...
  while ( (iter <= 100) && ((abs(Err(length(Err)))) > Tol) && (raiz_exacta == 0) && (introducido == 0) )
    ...
    elseif length(valores_c) == 1;
      valores_c(iter) = (valores_c(iter-1) - Fc(iter-1)*(valores_c(iter-1))/(Fc(iter-1)));
    elseif length(valores_c) >= 2;
      valores_c(iter) = (valores_c(iter-1) - Fc(iter-1)*(valores_c(iter-1) - valores_c(iter-2))/(Fc(iter-1) - Fc(iter-2)));
    endif
