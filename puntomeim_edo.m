...
function [Tiempo,y] = puntomeim_edo(f,tspan,valor_inicial,num_iter)
  ...
  for (iter = 1:num_iter)
    ...
    ym(iter+1,:) = fsolve(@(w) w - y(iter,:) - (h/2) * f(Tiempo(iter) + (h/2), w), ym(iter,:));
    
    y(iter+1,:) = fsolve(@(w) w - y(iter,:) - h * f(Tiempo(iter) + (h/2), ym(iter+1,:)), y(iter,:));
