...
function [c, iter] = biseccion(funcion,a,b,Tol)
  ...
  Err(1) = (valores_b(1)-valores_a(1))/2;
  ...
  while ( (iter <= 100) && ((abs(Err(length(Err)))) > Tol) && (raiz_exacta == 0) && (introducido == 0) )
    ...
    valores_c(iter) = (valores_a(iter)+valores_b(iter))/2;
    Err(iter) = (valores_b(iter)-valores_a(iter))/2;
    Err_rel(iter) = (valores_b(iter)-valores_a(iter))/(2*valores_c(iter));
